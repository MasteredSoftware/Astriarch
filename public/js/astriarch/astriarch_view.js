...
Astriarch.View.PageLoadInit = function(serverConfig) {
  ...
  Astriarch.View.jCanvasDraw = new jCanvas({
    ...
    layers: [
      ...
      {
        name: "canvasAstriarchPlayfield",
        x: 177,
        y: 21,
        width: 621,
        height: 480,
        zIndex: 2,
        clickCallback: Astriarch.View.PlayfieldClicked,
        dblclickCallback: Astriarch.View.PlayfieldDoubleClicked,
        mouseHitSelector: "#canvasMouseHitDetector"
      },
      {
        name: "canvasAstriarchFleets",
        x: 177,
        y: 21,
        width: 621,
        height: 480,
        zIndex: 3,
        clickCallback: Astriarch.View.FleetsClicked,
        mouseHitSelector: "#canvasMouseHitDetector"
      }
    ]
  });
  ...
  if (Astriarch.View.jCanvasDraw["canvasSupported"]) {
    ...
    setTimeout(function() {
      Astriarch.View.loadBackgroundSpriteSheetImage();
    }, 10); //give the browser a bit to render everything before loading and drawing

    Astriarch.View.ContextBackground.globalAlpha = 1.0;

    setInterval(function() {
      Astriarch.View.jCanvasDraw.draw();
    }, 50);
    ...
    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.NOOP, function(message) {
      console.log("NOOP: ", message);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.CHAT_ROOM_SESSIONS_UPDATED, function(message) {
      Astriarch.CommControl.setPlayerSessions(message.payload.sessions);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.LIST_GAMES, Astriarch.LobbyControl.refreshGameList);
    Astriarch.server_comm.register(
      Astriarch.Shared.MESSAGE_TYPE.GAME_LIST_UPDATED,
      Astriarch.LobbyControl.updateGameList
    );

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.CHANGE_GAME_OPTIONS, function(message) {
      Astriarch.NewGameControl.OnGameOptionsChanged(message);
    });
    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.JOIN_GAME, function(message) {
      Astriarch.LobbyControl.OnGameJoinMessageResponse(message);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.CREATE_GAME, function(message) {
      Astriarch.LobbyControl.OnGameCreateMessageResponse(message);
    });
    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.RESUME_GAME, function(message) {
      Astriarch.LobbyControl.OnGameResumeMessageResponse(message);
    });
    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.START_GAME, function(message) {
      Astriarch.NewGameControl.OnGameStartMessageResponse(message);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.END_TURN, function(message) {
      Astriarch.GameController.OnEndTurnMessageResponse(message);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.GAME_OVER, function(message) {
      Astriarch.GameController.OnGameOverMessageResponse(message);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.CHAT_MESSAGE, function(message) {
      Astriarch.CommControl.appendMessages([message.payload]);
    });

    Astriarch.server_comm.register(Astriarch.Shared.MESSAGE_TYPE.EXIT_RESIGN, function(message) {
      Astriarch.GameController.OnExitResignMessageResponse(message);
    });
...
Astriarch.View.RegisterHotkeys = function() {
  ...
  var selectHomePlanet = function() {
    Astriarch.View.selectPlanet(
      Astriarch.ClientGameModel.getClientPlanetById(Astriarch.ClientGameModel.MainPlayer.HomePlanetId)
    );
  };
  var selectNextPlanet = function() {
    Astriarch.View.cycleSelectedPlanet(1);
  };
  var selectPreviousPlanet = function() {
    Astriarch.View.cycleSelectedPlanet(-1);
  };
  var finishSendShips = function() {
...
Astriarch.View.CanvasNotSupported = function() {
  ...
  $("#canvasContainer").html(
    '<p style="color:white">Astriarch HTML 5 version not supported by your browser.  You can play with the silverlight plugin by clicking <a href="silverlight.html">here</a>. In 5 seconds you will be taken to the silverlight version automatically.</p>'
  );
  setTimeout(function() {
    window.location = "silverlight.html";
  }, 5000);
};
...
Astriarch.View.ClearGameBackgroundControls = function() {
  ...
  $(
    "#TurnDisplay,#OverallPlayerStatusGrid,#SelectedItemStatus,#SelectedItemPopulationPanel,#SelectedItemImprovementSlotsPanel,#SelectedItemPopulationAssignmentsPanel,#SelectedItemBuiltImprovementsGrid,#SelectedItemPlanetaryFleetGrid,#SelectedItemStatusDetails,#BottomStatusGrid,#TurnSummaryItemsListBox,#ButtonPanel"
  ).hide();
...
Astriarch.View.SetupGraphicalDOMElements = function() {
  ...
  Astriarch.View.TurnSummaryItemsListBox = new JSListBox({
    containerSelector: "TurnSummaryItemsListBox",
  ...
  $("#SystemMasterVersion").attr(
    "Title",
    "Astriarch - Ruler of the Stars, Version: " +
      Astriarch.Version +
  ...
  $("#SendShipsButton").click(function() {
  ...
  $("#MainMenuIcon").mouseenter(function() {
    Astriarch.View.mainMenuIconMouseEnter();
  });
  $("#MainMenuButton").mouseleave(function() {
    Astriarch.View.mainMenuButtonMouseLeave();
  });

  $("#SpeakerIcon").mouseenter(function() {
    Astriarch.View.speakerIconMouseEnter();
  });
  $("#ButtonSpeakerToggleMute").mouseleave(function() {
    Astriarch.View.muteButtonMouseLeave();
  });

  $("#SliderVolume").mouseenter(function() {
    Astriarch.View.volumeSliderMouseEnter();
  });
  $("#SliderVolume").mouseleave(function() {
    Astriarch.View.volumeSliderMouseLeave();
  });

  $("#ButtonSpeakerToggleMute").button({ icons: { primary: "icon-16x16-speaker-on" }, text: false });

  $("#ButtonSpeakerToggleMute").click(function() {
    Astriarch.View.toggleAudioMute();
  });

  $("#ButtonOpenTradingCenter").button({ icons: { primary: "icon-16x16-trading" }, text: false });

  $("#ButtonOpenTradingCenter").click(function() {
  ...
  $("#ButtonOpenResearch").click(function() {
  ...
  Astriarch.View.SpriteSheetInfo.planetAsteroidY =
    -1 * getPropertyValueFromCssClass("icon-32x32-PlanetAsteroid", "background-position");
  Astriarch.View.SpriteSheetInfo.planetDeadY =
    -1 * getPropertyValueFromCssClass("icon-32x32-PlanetDead", "background-position");
  Astriarch.View.SpriteSheetInfo.planetClass1Y =
    -1 * getPropertyValueFromCssClass("icon-32x32-PlanetClass1", "background-position");
  Astriarch.View.SpriteSheetInfo.planetClass2Y =
    -1 * getPropertyValueFromCssClass("icon-32x32-PlanetClass2", "background-position");
  Astriarch.View.SpriteSheetInfo.starshipDefenderY =
    -1 * getPropertyValueFromCssClass("icon-32x32-DefenderLarge", "background-position");
  Astriarch.View.SpriteSheetInfo.starshipScoutY =
    -1 * getPropertyValueFromCssClass("icon-32x32-ScoutLarge", "background-position");
  Astriarch.View.SpriteSheetInfo.starshipDestroyerY =
    -1 * getPropertyValueFromCssClass("icon-32x32-DestroyerLarge", "background-position");
  Astriarch.View.SpriteSheetInfo.starshipCruiserY =
  ...
  var shipColorRgbaArray = [
    ...
    Astriarch.View.ColorsRGBA["yellow"],
    Astriarch.View.ColorsRGBA["orange"],
    Astriarch.View.ColorsRGBA["red"]
  ];
  Astriarch.View.ImageCanvasStarships.defender = new Astriarch.View.StarshipCanvas(
    "#CanvasDefenderLarge",
  ...
  Astriarch.View.ImageCanvasStarships.scout = new Astriarch.View.StarshipCanvas(
    "#CanvasScoutLarge",
    Astriarch.View.SpriteSheetInfo.filename,
    32,
    32,
  ...
  Astriarch.View.ImageCanvasStarships.destroyer = new Astriarch.View.StarshipCanvas(
    "#CanvasDestroyerLarge",
    Astriarch.View.SpriteSheetInfo.filename,
    32,
    32,
    shipColorRgbaArray,
  ...
  Astriarch.View.ImageCanvasStarships.cruiser = new Astriarch.View.StarshipCanvas(
    "#CanvasCruiserLarge",
    Astriarch.View.SpriteSheetInfo.filename,
    32,
    32,
  ...
  Astriarch.View.ImageCanvasStarships.battleship = new Astriarch.View.StarshipCanvas(
    "#CanvasBattleshipLarge",
    Astriarch.View.SpriteSheetInfo.filename,
    32,
    32,
    shipColorRgbaArray,
...
Astriarch.View.FinishSendShips = function() {
  ...
  if (Astriarch.View.sendingShipsTo) {
    if (Astriarch.View.sendShipsFromHex != Astriarch.ClientGameModel.GameGrid.SelectedHex) {
      var distance = Astriarch.ClientGameModel.GameGrid.GetHexDistance(
        Astriarch.View.sendShipsFromHex,
        Astriarch.ClientGameModel.GameGrid.SelectedHex
      );
...
Astriarch.View.updatePlayerStatusPanel = function() {
  ...
  if (mainPlayer !== null) {
    ...
    if (foodDiffPerTurn >= 0) foodDiffPositiveIndicator = "+";
    ...
    $("#TextBlockFoodAmount").text(
      Math.floor(totalFoodAmount) + " " + foodDiffPositiveIndicator + foodDiffPerTurn.toFixed(decimalPlaces)
    );
    $("#TextBlockFoodAmount").prop(
      "title",
      "Food Amount: " +
        totalFoodAmount.toFixed(1) +
        " +" +
        totalResourceProduction.food.toFixed(1) +
        " -" +
        totalPopulation
    );
    ...
    if (goldDiffPerTurn >= 0) goldDiffPositiveIndicator = "+";
    ...
    $("#TextBlockGoldAmount").text(
      Math.floor(mainPlayer.Resources.GoldAmount) +
        " " +
        goldDiffPositiveIndicator +
        goldDiffPerTurn.toFixed(decimalPlaces)
    );
    $("#TextBlockGoldAmount").prop(
      "title",
      "Gold Amount: " +
        mainPlayer.Resources.GoldAmount.toFixed(1) +
        " +" +
        totalResourceProduction.gold.toFixed(1) +
        " -" +
        totalResourceProduction.foodToShip
    );
    ...
    $("#TextBlockOreAmount").prop(
      "title",
      "Ore Amount: " + mainPlayer.TotalOreAmount().toFixed(1) + " +" + totalResourceProduction.ore.toFixed(2)
    );

    var iridiumAmount = mainPlayer.TotalIridiumAmount();
    decimalPlaces = iridiumAmount >= 10 ? 0 : 1;
    $("#TextBlockIridiumAmount").text(
      Math.floor(iridiumAmount) + " +" + totalResourceProduction.iridium.toFixed(decimalPlaces)
    );
    $("#TextBlockIridiumAmount").prop(
      "title",
      "Iridium Amount: " +
        mainPlayer.TotalIridiumAmount().toFixed(1) +
        " +" +
        totalResourceProduction.iridium.toFixed(2)
    );
  }
};
...
Astriarch.View.updateSelectedItemPanelForPlanet = function() {
  ...
  if (Astriarch.ClientGameModel.GameGrid.SelectedHex != null) {
    ...
    if (!planetTypeIfKnownByPlayer) {
      //this planet is unexplored
      sb += "Unexplored";
    } //the main player has explored it
    else {
      ...
      if (p) {
        ...
        if (p.Owner != null) owner = p.Owner.Name;
        sb += owner + "<br />";
        ...
        if (ppi) {
          ...
          } else if (ppi instanceof Astriarch.Planet.StarShipInProduction) {
            //it's a ship
            queuedItemClass = "qi-starShipInProduction";
          } else if (ppi instanceof Astriarch.Planet.PlanetImprovementToDestroy) {
        ...
        $("#TextBlockDefenderCount").text(
          p.PlanetaryFleet.StarShips[Astriarch.Fleet.StarShipType.SystemDefense].length
        );
        ...
        if (p.Owner == mainPlayer) {
          //make sure we can't modify planets even while debugging TODO: hack for now will have to change for multi-player
        ...
        sisdText +=
          "Production: " + Astriarch.DecimalToFixed(p.ResourcesPerTurn.GetProductionAmountPerTurn(), 3) + "<br />";
        ...
        sisdText +=
          "Production: " +
          Astriarch.DecimalToFixed(p.ResourcesPerTurn.GetExactProductionAmountPerWorkerPerTurn(), 3) +
          "<br />";
        sisdText +=
          "Food: " + Astriarch.DecimalToFixed(p.ResourcesPerTurn.GetExactFoodAmountPerWorkerPerTurn(), 3) + "<br />";
        sisdText +=
          "Ore: " + Astriarch.DecimalToFixed(p.ResourcesPerTurn.GetExactOreAmountPerWorkerPerTurn(), 3) + "<br />";
        sisdText +=
          "Iridium: " +
          Astriarch.DecimalToFixed(p.ResourcesPerTurn.GetExactIridiumAmountPerWorkerPerTurn(), 3) +
          "<br />";
        ...
        $("#TextBlockDestroyerCount").text(
          lastKnownPlanetFleet.StarShips[Astriarch.Fleet.StarShipType.Destroyer].length
        );
        $("#TextBlockCruiserCount").text(lastKnownPlanetFleet.StarShips[Astriarch.Fleet.StarShipType.Cruiser].length);
        $("#TextBlockBattleshipCount").text(
          lastKnownPlanetFleet.StarShips[Astriarch.Fleet.StarShipType.Battleship].length
        );
...
Astriarch.View.updateSelectedItemPopulationPanel = function(planet) {
  ...
  for (var i = maxPopulation + 1; i <= 16; i++) {
    ...
    element.prop(
      "title",
      "Planet Population: " + population.length + " / " + maxPopulation + popGrowthInText + planetHappinessText
    );
  }
  ...
  for (var i = 1; i <= maxPopulation; i++) {
    ...
    if (i <= population.length) {
      ...
      if (citizen.ProtestLevel == 0) {
        ...
        element.prop(
          "title",
          "Citizens Protesting at " +
            Math.round(100 * citizen.ProtestLevel) +
            "%" +
            popGrowthInText +
            planetHappinessText
        );
...
Astriarch.View.updateSelectedItemImprovementSlotsPanel = function(improvementCount, maxImprovements) {
  $("#SelectedItemImprovementSlotsPanel").attr(
    "Title",
    "Planetary Improvements: " + improvementCount + " / " + maxImprovements
  );
  ...
  for (var i = 1; i <= maxImprovements; i++) {
    ...
    else $("#PlanetaryImprovementSlotsImage" + i).addClass("icon-16x16-PlanetaryImprovementSlotEmpty");
  }
};
...
Astriarch.View.backgroundSpriteSheetImageLoaded = function() {
  ...
  var preStepCallback = function(index) {
    window.tour.enabled = true;
    window.tour.step = index;
  };
  var postStepCallback = function(index, tip) {};
  var preTutorialCallback = function(index, tip, elm) {};
  var postTutorialCallback = function() {
    window.tour.enabled = false;
  };
  ...
  for (var row = 0; row < rows; row += 2) {
    for (var col = 0; col < cols; col += 2) {
      if (Astriarch.NextRandom(0, 2) == 0) {
        ...
        //set image opacity
        Astriarch.View.ContextBackground.globalAlpha =
          Astriarch.NextRandom(70 - Astriarch.View.BACKGROUND_DARKNESS, 101 - Astriarch.View.BACKGROUND_DARKNESS) /
          100.0;
        //use spritesheet co-ordinates:
        Astriarch.View.ContextBackground.drawImage(
          Astriarch.View.BackgroundSpriteSheetImage,
          0,
          imageIndex * RECT_HEIGHT,
          RECT_WIDTH,
          RECT_HEIGHT,
          col * RECT_WIDTH + offsetX,
          row * RECT_HEIGHT + offsetY,
          RECT_WIDTH,
          RECT_HEIGHT
        );
      }
...
Astriarch.View.populateStars = function() {
  ...
  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < cols; col++) {
      ...
      for (var starNum = 0; starNum < starCount; starNum++) {
        ...
        var starSize = 1.0;
        if (starNum >= 6 || starNum % 2 == 0) {
          //prefer small white stars
          var opacity =
            Astriarch.NextRandom(40 - Astriarch.View.BACKGROUND_DARKNESS, 101 - Astriarch.View.BACKGROUND_DARKNESS) /
            140.0;
  ...
  var backgroundImg = Astriarch.View.ContextBackground.createImageData(
    Astriarch.View.CanvasBackground[0].width,
    Astriarch.View.CanvasBackground[0].height
  );
...
Astriarch.View.paintLargeStars = function(largeStars) {
  for (var i = 0; i < largeStars.length; i++) {
    ...
    Astriarch.View.ContextBackground.globalAlpha =
      Astriarch.NextRandom(50 - Astriarch.View.BACKGROUND_DARKNESS, 101 - Astriarch.View.BACKGROUND_DARKNESS) / 100.0;
...
Astriarch.View.toggleAudioMute = function() {
  ...
  if (Astriarch.View.audioInterface.muted) {
    $("#SpeakerIcon")
      .removeClass("icon-16x16-speaker-on")
      .addClass("icon-16x16-speaker-off");
    $("#ButtonSpeakerToggleMute > .ui-icon")
      .removeClass("icon-16x16-speaker-on")
      .addClass("icon-16x16-speaker-off");
  } else {
    $("#SpeakerIcon")
      .removeClass("icon-16x16-speaker-off")
      .addClass("icon-16x16-speaker-on");
    $("#ButtonSpeakerToggleMute > .ui-icon")
      .removeClass("icon-16x16-speaker-off")
      .addClass("icon-16x16-speaker-on");
  }
