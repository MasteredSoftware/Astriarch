Astriarch.SendShipsControl = {
  ...
  init: function() {
    Astriarch.SendShipsControl.StarShipsAvailableCardList = new JSCardList({
      containerSelector: "StarShipsAvailableCardList",
      multiselect: true
    });

    $("#ButtonSendNoShips, #ButtonSendAllShips").button();

    $("#ButtonSendNoShips").click(function() {
      Astriarch.SendShipsControl.ButtonSendNoShipsClick();
    });

    $("#ButtonSendAllShips").click(function() {
      Astriarch.SendShipsControl.ButtonSendAllShipsClick();
    });

    var checkBoxTooltip =
      "When a waypoint is set on a planet, newly built ships will automatically be sent to the destination planet.";
    $("#SetWaypointCheckBoxLabel").attr("title", checkBoxTooltip);
    $("#SetWaypointCheckBox").prop("checked", false);

    Astriarch.SendShipsControl.dialog = new Astriarch.Dialog(
      "#sendShipsDialog",
      "Send Ships",
      380,
      330,
      Astriarch.SendShipsControl.OKClose,
      Astriarch.SendShipsControl.CancelClose
    );
  },
  ...
  OKClose: function() {
    ...
    if (selectedItems.length > 0) {
      ...
      self.CreatedFleet.CreateDrawnFleetAndSetDestination(
        Astriarch.ClientGameModel.GameGrid,
        self.pSource.BoundingHex,
        self.pDest.BoundingHex
      );

      self.pSource.OutgoingFleets.push(self.CreatedFleet);

      var payload = {
        planetIdSource: self.pSource.Id,
        planetIdDest: self.pDest.Id,
        data: { scouts: scoutIds, destroyers: destroyerIds, cruisers: cruiserIds, battleships: battleshipIds }
      };
...
Astriarch.SendShipsControl.AvailableStarShipCardListItem = JSCardList.Item.extend({
  ...
  render: function() {
    ...
    var element =
      '<span class="sscItem">' +
      '<span class="sscItemHealth">' +
      details.damageText +
      "</span>" +
      '<div class="sscItemImg ' +
      imageClassName +
      '" />' +
      '<div class="sscHealthBarContainer"><div class="sscHealthBar" style="background:' +
      this.Foreground +
      ";height:" +
      details.percentHealthText +
      '" /></div>' +
      '<div class="sscExpBarContainer"><div class="sscExpBar" style="background:#369;height:' +
      percentLevel * 100 +
      '%" /></div>' +
      '<span class="sscItemLevel">' +
      (levelObj.level + 1) +
      "</span>" +
      "</span>";

    return (
      '<a class="sscItemAnchor" href="#" title="' +
      this.Tooltip +
      '" style="color:' +
      this.Foreground +
      '">' +
