 ...
 * @constructor
 */
Astriarch.DrawnPlanet = jCanvas.DrawnObject.extend({
  // drawn object class
  ...
  init: function(/*ClientPlanet*/ p) {
    ...
    this.fleetRectangle = new Astriarch.Rectangle(
      this.ClientPlanet.BoundingHex.MidPoint.X + Astriarch.Planet.Static.PLANET_SIZE / 2 - 2,
    ...
    this.spacePlatformRectangle = new Astriarch.Rectangle(
      this.ClientPlanet.BoundingHex.MidPoint.X - Astriarch.Planet.Static.PLANET_SIZE / 2 - 8,
      this.ClientPlanet.BoundingHex.MidPoint.Y + Astriarch.Planet.Static.PLANET_SIZE / 2 - 2,
      11,
      11
    );
    ...
    if (this.planetImageBackgroundPosition === null) {
      ...
      image.onload = function() {
        ...
        ctx.drawImage(
          image,
          0,
          thisDrawnPlanet.planetImageBackgroundPosition,
          tileSize,
          tileSize,
          x,
          y,
      ...
      if (this.drawFleetRectangle) {
        ...
        for (var i in this.fleetRectangleImageData) starshipImg.data[i] = this.fleetRectangleImageData[i];
      ...
      if (this.productionItemStatusColor) {
        ...
        // draw left side of Ellipse
        ctx.bezierCurveTo(
          statusCenterX - statusRadius,
          statusCenterY - statusRadius,
          statusCenterX - statusRadius,
          statusCenterY + statusRadius,
          statusCenterX,
          statusCenterY + statusRadius
  ...
  UpdatePlanetDrawingForPlayer: function(/*Astriarch.ClientGameModel*/ clientGameModel) {
    ...
    if (planet) {
      ...
      if (planet.WayPointPlanetId) {
        ...
        if (wayPointPlanet) {
          this.wayPointLine = new Astriarch.Line(
            this.ClientPlanet.BoundingHex.MidPoint.X,
      ...
      if (this.knownPlanetType == Astriarch.Planet.PlanetType.DeadPlanet) this.textBlockForeground = "black";
      else this.textBlockForeground = "yellow";

      this.textBlockStrengthForeground = "yellow";

      if (this.knownPlanetType && lastKnownFleet)
        this.textBlockStrengthText = lastKnownFleet.Fleet.DetermineFleetStrength() + "";
      else this.textBlockStrengthText = "";
    }
  }
});
