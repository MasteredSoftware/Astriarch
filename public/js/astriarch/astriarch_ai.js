...
Astriarch.AI = {
  OnComputerSentFleet: function(f) {
    console.debug(
      ...
      f.ToString(),
      "From:",
      f.travelingFromHex.PlanetContainedInHex.Name,
      "To:",
      f.DestinationHex.PlanetContainedInHex.Name
    );
  },
  ...
  computerAdjustPopulationAssignments: function(/*Player*/ player) {
    ...
    for (var i in player.OwnedPlanets) {
      ...
      if (p.Population.length < p.MaxPopulation()) totalPlanetsWithPopulationGrowthPotential++;
    }
    ...
    var totalFoodAmountOnPlanetsAdjustment = Astriarch.NextRandom(
    ...
    var iridiumAmountNeeded =
      Math.round(iridiumAmountRecommended * mineralOverestimation) - player.TotalIridiumAmount();
    var foodDiff = 0;
    ...
    if (totalPopulation > totalFoodProduction + totalFoodAmountOnPlanets) {
      ...
      if (neededFarmers > 0)
        for (var i in allPlanets) {
          ...
          if (
            neededFarmers > 0 &&
            p.ResourcesPerTurn.GetExactFoodAmountNextWorkerPerTurn() > 0 &&
      ...
      while (foodDiff > 0) {
        ...
        for (var i in planetCandidatesForAddingFarmers) {
          ...
          if (p.Type == Astriarch.Planet.PlanetType.PlanetClass2) {
            if (p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Mine].length == 0) maxMiners = 0;
            else maxMiners = 1;
          }
      ...
      while (foodDiff > 0) {
        ...
        for (var i in planetCandidatesForRemovingFarmers) {
          ...
          if (foodDiff < p.ResourcesPerTurn.GetExactFoodAmountPerWorkerPerTurn()) continue; //if removing this farmer would create a shortage, skip this planet
          ...
          var addMiner =
            p.Type != Astriarch.Planet.PlanetType.PlanetClass2 ||
            p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Mine].length > 0;
    ...
    if (oreAmountNeeded > 0 || iridiumAmountNeeded > 0) {
      ...
      var mineralProductionComparer = new Astriarch.Planet.PlanetMineralProductionPotentialComparer(
        oreAmountNeeded,
        iridiumAmountNeeded
      );
      ...
      while (oreAmountNeeded > 0 || iridiumAmountNeeded > 0) {
        ...
        for (var i in planetCandidatesForRemovingWorkers) {
          ...
          var minWorkers = 1;
          if (p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Mine].length == 0) minWorkers = 2;
      ...
      while (oreAmountNeeded < 0 && iridiumAmountNeeded < 0) {
        ...
        for (var i in planetCandidatesForRemovingMiners) {
          ...
          if (
            planetMiners[p.Id] > 0 &&
            (oreAmountNeeded + p.ResourcesPerTurn.GetExactOreAmountPerWorkerPerTurn() < 0 &&
              iridiumAmountNeeded + p.ResourcesPerTurn.GetExactIridiumAmountPerWorkerPerTurn() < 0)
          ) {
            p.UpdatePopulationWorkerTypesByDiff(planetFarmers[p.Id], planetMiners[p.Id], planetWorkers[p.Id], 0, -1, 1);
            planetMiners[p.Id]--;
            planetWorkers[p.Id]++;
  ...
  computerSetPlanetBuildGoals: function(gameModel, /*Player*/ player, ownedPlanetsSorted) {
      ...
      if (!player.PlanetBuildGoals[p.Id]) {
        ...
        if (p.BuildQueue.length <= 1) {
          ...
          if (ownedPlanetsSorted.length > 1) {
            if (
              p.GetSpacePlatformCount() < player.Research.getMaxSpacePlatformCount() &&
              p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Factory].length > 0
            ) {
    ...
    for (var i in planetCandidatesForNeedingSpacePlatforms) {
      ...
      if (p.GetSpacePlatformCount(true) <= player.Research.getMaxSpacePlatformCount()) {
        player.PlanetBuildGoals[p.Id] = new Astriarch.Planet.StarShipInProduction(
    ...
    for (var i in planetCandidatesForNeedingImprovements) {
      ...
      if (farmCount < recommendedFarms) {
        player.PlanetBuildGoals[p.Id] = new Astriarch.Planet.PlanetImprovement(
        ...
        player.PlanetBuildGoals[p.Id] = new Astriarch.Planet.PlanetImprovementToDestroy(
          Astriarch.Planet.PlanetImprovementType.Farm
        );
      } else if (mineCount > recommendedMines) {
        player.PlanetBuildGoals[p.Id] = new Astriarch.Planet.PlanetImprovementToDestroy(
      ...
      if (player.PlanetBuildGoals[p.Id]) {
        console.debug(
          ...
          Astriarch.GameTools.PlanetImprovementTypeToFriendlyName(player.PlanetBuildGoals[p.Id].Type)
        );
      }
    ...
    for (var i in planetCandidatesForNeedingShips) {
      ...
      if (player.PlanetBuildGoals[p.Id]) {
        console.debug(
          ...
          p.Name,
          "StarShip Build Goal:",
          Astriarch.GameTools.StarShipTypeToFriendlyName(player.PlanetBuildGoals[p.Id].Type)
        );
      }
    }
  },
      ...
      if (totalOre >= oreDesired * 2) {
        ...
        tradesToExecute.push(
          new Astriarch.TradingCenter.Trade(
            ...
            planetId,
            tradeType,
            Astriarch.TradingCenter.ResourceType.ORE,
            amount,
            orderType,
            limitPrice
          )
      ...
      if (totalIridium >= iridiumDesired * 2) {
        ...
        tradesToExecute.push(
          new Astriarch.TradingCenter.Trade(
            ...
            planetId,
            tradeType,
            Astriarch.TradingCenter.ResourceType.IRIDIUM,
            amount,
            orderType,
            limitPrice
          )
    ...
    for (var i in ownedPlanetsSorted) {
      ...
      if (p.BuildQueue.length == 0) {
        if (p.Id in player.PlanetBuildGoals) {
          ...
          //check resources
          if (
            player.Resources.GoldAmount - ppi.GoldCost > goldSurplus &&
  ...
  computerSendShips: function(gameModel, /*Player*/ player, ownedPlanetsSorted) {
      ...
      if (p.PlanetaryFleet.GetPlanetaryFleetMobileStarshipCount() > 0) {
        if (player.Type == Astriarch.Player.PlayerType.Computer_Easy) {
          ...
          if (Astriarch.AI.countPlanetsNeedingExploration(gameModel, player) != 0) {
            ...
            strengthToDefend = 0;
          } else if (p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Factory].length > 0) {
          ...
          if (
            player.Type == Astriarch.Player.PlayerType.Computer_Hard ||
            player.Type == Astriarch.Player.PlayerType.Computer_Expert
          ) {
            ...
            if (closestUnownedPlanet != null) {
              if (player.LastKnownPlanetFleetStrength[closestUnownedPlanet.Id]) {
                strengthToDefend += player.LastKnownPlanetFleetStrength[
                  closestUnownedPlanet.Id
                ].Fleet.DetermineFleetStrength(true);
            ...
            if (p.BuildQueueContainsMobileStarship(turnsToCompleteStarshipStrengthObject)) {
              if (turnsToCompleteStarshipStrengthObject["turnsToComplete"] <= minDistanceObject["minDistance"] + 1) {
    ...
    if (player.HomePlanetId != null) {
      ...
      if (
        ...
        var planetValueDistanceStrengthComparer = new Astriarch.Planet.PlanetValueDistanceStrengthComparer(
          gameModel,
          homePlanet,
    ...
    for (var i = planetCandidatesForInboundAttackingFleets.length - 1; i >= 0; i--) {
      ...
      var additionalStrengthMultiplierNeededToAttack =
        Astriarch.NextRandom(
      ...
      for (var j = planetCandidatesForSendingShips.length - 1; j >= 0; j--) {
        ...
        var newFleet = Astriarch.Fleet.StarShipFactoryHelper.GenerateFleetWithShipCount(
          player,
          0,
          scouts,
          destroyers,
      ...
      if (!fleetSent && planetCandidatesForInboundReinforcements.length > 0) {
        ...
        var planetToReinforce =
          planetCandidatesForInboundReinforcements[planetCandidatesForInboundReinforcements.length - 1];
        var distanceFromPlanetToReinforceToEnemy = gameModel.GameGrid.GetHexDistance(
          pEnemyInbound.BoundingHex,
          planetToReinforce.BoundingHex
        );
        ...
        for (var r = planetCandidatesForSendingShips.length - 1; r >= 0; r--) {
          ...
          if (
            gameModel.GameGrid.GetHexDistance(pEnemyInbound.BoundingHex, pFriendly.BoundingHex) <
  ...
  getClosestUnownedPlanet: function(gameModel, /*Player*/ player, /*Planet*/ playerOwnedPlanet, minDistanceObject) {
    //returns Planet
  ...
  planetNeedsExploration: function(p, gameModel, player) {
    ...
    var turnsSinceLastExplored =
      p.Id in player.LastKnownPlanetFleetStrength
        ? gameModel.Turn.Number - player.LastKnownPlanetFleetStrength[p.Id].TurnLastExplored
        : 0;
