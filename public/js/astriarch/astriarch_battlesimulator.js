...
Astriarch.BattleSimulator = {
	...
	SimulateFleetBattle: function(/*Fleet*/ f1, /*Fleet*/ f2){//returns bool?
		...
		while (f1.DetermineFleetStrength() > 0 && f2.DetermineFleetStrength() > 0) {
			...
			for (var i in f1StarShips) {
				...
				experiencedGainedByStarShipId[s.id] += Astriarch.BattleSimulator.StarshipFireWeapons(f1BonusChance.attack, f2BonusChance.defense, s, f2StarShips, fleet2DamagePending);
			}
			...
			for (var i in f2StarShips) {
				...
				experiencedGainedByStarShipId[s.id] += Astriarch.BattleSimulator.StarshipFireWeapons(f2BonusChance.attack, f1BonusChance.defense, s, f1StarShips, fleet1DamagePending);
			}
	...
	StarshipFireWeapons: function(attackBonusChance, defenseBonusChance, /*StarShip*/ ship, /*List<StarShip>*/ enemyFleet, /*Dictionary<StarShipId, {Starship:starshipObject, Damage:int}>*/ fleetDamagePending) {
		var totalDamage = 0;
		var damage = 0;
		var maxDamage = 0;
