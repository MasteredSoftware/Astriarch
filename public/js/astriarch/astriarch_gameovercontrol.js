Astriarch.GameOverControl = {
  ...
  init: function() {
    Astriarch.GameOverControl.dialog = new Astriarch.Dialog(
      "#gameOverDialog",
      "Game Over",
      424,
      313,
      Astriarch.GameOverControl.OKClose
    );
  },
  ...
  show: function(/*SerializablePlayer*/ winningSerializablePlayer, /*bool*/ localPlayerWon, score) {
    ...
    if (localPlayerWon)
      summary +=
        "You conquered all of your enemies and reign supreme over the known universe!<br />You will be known as the first Astriarch - Ruler of the Stars.<br />";
    else
      summary +=
        "You lost control over all your fleets and planets and have been crushed by the power of your enemies!<br />";

    summary += "In " + Astriarch.ClientGameModel.Turn.Number + " turns.<br />";
    if (winningSerializablePlayer) {
      summary +=
        winningSerializablePlayer.Name +
        " won the game with " +
        winningSerializablePlayer.OwnedPlanetIds.length +
        " planets.<br />";
    ...
    AstriarchExtern.OnGameOver({
      PlayerWon: localPlayerWon,
      Turns: Astriarch.ClientGameModel.Turn.Number,
      Score: score
    });
