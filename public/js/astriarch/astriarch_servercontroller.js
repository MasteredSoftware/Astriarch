...
Astriarch.ServerController = {
  ...
  EndTurns: function(gameModel) {
    //Dictionary<Id, SerializableTurnEventMessage>
    ...
    for (var i in gameModel.Players) {
      var player = gameModel.Players[i];
      if (player.Type == Astriarch.Player.PlayerType.Human) {
        endOfTurnMessagesByPlayerId[player.Id] = endOfTurnMessagesByPlayerId[player.Id].concat(
    ...
    for (var i in gameModel.Planets) {
      ...
      if (
        p.Owner &&
        p.PlanetHappiness == Astriarch.Planet.PlanetHappinessType.Normal &&
        p.BuiltImprovements[Astriarch.Planet.PlanetImprovementType.Colony].length > 0
      ) {
        ...
        var maxStrengthToRepair = Math.min(
          Math.floor((goldAmount / 2) * 3),
          Math.floor(oreAmount * 2),
          Math.floor(iridiumAmount * 4)
        );
    ...
    for (var playerId in resourcesAutoSpentByPlayerId) {
      ...
      if (!messageFound) {
        endOfTurnMessagesByPlayerId[playerId].push(
          new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.ResourcesAutoSpent,
            null,
            text
          )
        );
  ...
  endTurn: function(gameModel, /*Player*/ player) {
    ...
    if (player.Type == Astriarch.Player.PlayerType.Human) {
      endOfTurnMessages = endOfTurnMessages.concat(
    ...
    endOfTurnMessages = endOfTurnMessages.concat(
      Astriarch.ServerController.buildPlayerPlanetImprovements(gameModel, player)
    );
    ...
    for (var pId in executedStatusListByPlayerId) {
      ...
      if (resourcesBought.tradeCount || resourcesSold.tradeCount) {
        ...
        if (resourcesBought.goldSpent) {
          message +=
            " Spent " +
            Astriarch.DecimalToFixed(resourcesBought.goldSpent, 2) +
            " gold buying: " +
        ...
        if (resourcesSold.goldEarned) {
          message +=
            " Earned " +
            Astriarch.DecimalToFixed(resourcesSold.goldEarned, 2) +
            " gold selling: " +
        ...
        endOfTurnMessagesByPlayerId[pId].push(
          new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.TradesExecuted,
            null,
            message
          )
        );
      ...
      if (resourcesNotBought.tradeCount || resourcesNotSold.tradeCount) {
        ...
        endOfTurnMessagesByPlayerId[pId].push(
          new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.TradesNotExecuted,
            null,
            message
          )
        );
  ...
  resolvePlanetaryConflicts: function(gameModel, /*Player*/ player, endOfTurnMessagesByPlayerId) {
    //returns List<SerializableTurnEventMessage>
    ...
    for (var i in unownedPlanetFleets) {
      ...
      var planetaryConflictData = new Astriarch.TurnEventMessage.PlanetaryConflictData(
      ...
      planetaryConflictData.AttackingFleetResearchBoostAttack = player.Research.getResearchData(
        Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_ATTACK
      ).chance;
      planetaryConflictData.AttackingFleetResearchBoostDefense = player.Research.getResearchData(
        Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_DEFENSE
      ).chance;
      planetaryConflictData.DefendingFleetResearchBoostAttack = destinationPlanet.Owner
        ? destinationPlanet.Owner.Research.getResearchData(Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_ATTACK)
            .chance
        : 0;
      planetaryConflictData.DefendingFleetResearchBoostDefense = destinationPlanet.Owner
        ? destinationPlanet.Owner.Research.getResearchData(Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_DEFENSE)
            .chance
        : 0;
      ...
      if (enemyFleetStrength > playerFleetStrength * Astriarch.ServerController.BATTLE_RANDOMNESS_FACTOR) {
        ...
        if (playerFleetStrength > enemyFleetStrength) {
          //prefer player
          var extraPercentageChance =
            (Math.log(playerFleetStrength / (enemyFleetStrength * 1.0)) /
              Math.log(Math.pow(Astriarch.ServerController.BATTLE_RANDOMNESS_FACTOR, 2))) *
          ...
          //prefer enemy
          var extraPercentageChanceEnemy =
      ...
      if (!playerWins) {
        ...
        if (player.Type == Astriarch.Player.PlayerType.Human) {
          ...
          if (destinationPlanet.Owner != null)
            message =
              "You lost a fleet attacking " + destinationPlanet.Owner.Name + " at planet: " + destinationPlanet.Name;
          var tem = new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.AttackingFleetLost,
            destinationPlanet,
            message
          );
        ...
        if (destinationPlanet.Owner && destinationPlanet.Owner.Type == Astriarch.Player.PlayerType.Human) {
          ...
          var tem = new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.DefendedAgainstAttackingFleet,
            destinationPlanet,
            message
          );
          planetaryConflictData.WinningFleet = enemyFleet.CloneFleet();
        ...
        if (defendingPlayer) {
          ...
          if (defendingPlayer.Resources.GoldAmount < goldLootMax) goldLootMax = defendingPlayer.Resources.GoldAmount;
          ...
          for (var i = 0; i < potentialResearchToSteal.length; i++) {
            ...
            if (
              attackingPlayerResearch.canResearch() &&
              !rp.isCustomShip &&
              ...
              planetaryConflictData.ResearchAmountLooted = Astriarch.NextRandom(
                0,
                Math.floor(
                  Math.min(
                    goldLootMax * 4,
              ...
              var levelIncrease = attackingPlayerResearch.setResearchPointsCompleted(
                attackingPlayerResearch.researchPointsCompleted + planetaryConflictData.ResearchAmountLooted
              );
              if (levelIncrease) {
                endOfTurnMessagesByPlayerId[defendingPlayer.Id].push(
                  new Astriarch.SerializableTurnEventMessage(
                    Astriarch.TurnEventMessage.TurnEventMessageType.ResearchStolen,
                    destinationPlanet,
                    attackingPlayerResearch.ToString() + " was stolen by " + player.Name
                  )
                );
                endOfTurnMessagesByPlayerId[player.Id].push(
                  new Astriarch.SerializableTurnEventMessage(
                    Astriarch.TurnEventMessage.TurnEventMessageType.ResearchStolen,
                    destinationPlanet,
                    "You stole " + attackingPlayerResearch.ToString() + " from " + defendingPlayer.Name
                  )
                );
              }
              break;
        ...
        if (player.Type == Astriarch.Player.PlayerType.Human) {
          ...
          var tem = new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.PlanetCaptured,
            destinationPlanet,
            message
          );
        ...
        if (defendingPlayer && defendingPlayer.Type == Astriarch.Player.PlayerType.Human) {
          ...
          var tem = new Astriarch.SerializableTurnEventMessage(
            Astriarch.TurnEventMessage.TurnEventMessageType.PlanetLost,
            destinationPlanet,
            message
          );
          planetaryConflictData.WinningFleet = playerFleet.CloneFleet();
  ...
  eatAndStarve: function(gameModel, /*Player*/ player, /*int*/ totalPop) {
    ...
    for (var i in player.OwnedPlanets) {
      ...
      } else if (p.Resources.FoodAmount > 0) {
    ...
    for (var planetId in foodDeficitByPlanet) {
      ...
      if (player.Resources.GoldAmount > 0 && foodSurplusPlanets.length > 0) {
        //it will cost one gold per resource shipped
        //look for a planet to send food from
        for (var s = 0; s < foodSurplusPlanets.length; s++) {
          var pSurplus = foodSurplusPlanets[s]; //Planet
          var amountSpent = 0;

          amountSpent = player.Resources.SpendGoldAsPossible(
            Math.min(foodShortageTotal, pSurplus.Resources.FoodAmount)
          );
          ...
          if (pSurplus.Resources.FoodAmount == 0) {
            //remove it for faster processing
      ...
      if (!shippedAllResources) {
        ...
        if (looseOne) {
          ...
          eotMessages.push(
            new Astriarch.SerializableTurnEventMessage(
              Astriarch.TurnEventMessage.TurnEventMessageType.FoodShortageRiots,
              planet,
          ...
          if (planet.Population.length > 0) {
            ...
            if (c.PopulationChange <= -1.0) {
              ...
              eotMessages.push(
                new Astriarch.SerializableTurnEventMessage(
                  Astriarch.TurnEventMessage.TurnEventMessageType.PopulationStarvation,
                  planet,
        ...
        for (var c = 0; c < planet.Population.length; c++) {
          ...
          if (Astriarch.NextRandom(0, protestDenominator) == 0) {
        ...
        if (planet.Population.length == 0) {
          ...
          eotMessages.push(
            new Astriarch.SerializableTurnEventMessage(
              Astriarch.TurnEventMessage.TurnEventMessageType.PlanetLostDueToStarvation,
              planet,
    ...
    if (protestingPlanetCount > 0) {
      ...
      eotMessages.push(
        new Astriarch.SerializableTurnEventMessage(
          Astriarch.TurnEventMessage.TurnEventMessageType.InsufficientFood,
    ...
    if (totalFoodShipped != 0) {
      eotMessages.push(
        new Astriarch.SerializableTurnEventMessage(
          Astriarch.TurnEventMessage.TurnEventMessageType.ResourcesAutoSpent,
          null,
          totalFoodShipped + " Gold spent shipping Food"
        )
      );
    }
  ...
  generatePlayerResourcesAndResearch: function(/*Player*/ player) {
    //returns List<TurnEventMessage>
    //perform research
  ...
  addLastStarShipToQueueOnPlanets: function(gameModel, /*Player*/ player) {
    ...
    for (var i in player.OwnedPlanets) {
      ...
      if (p.BuildLastStarShip && p.BuildQueue.length == 0 && p.StarShipTypeLastBuilt != null) {
        ...
        var s = new Astriarch.Planet.StarShipInProduction(
          p.StarShipTypeLastBuilt,
          p.StarShipCustomShipLastBuilt,
        ...
        if (canBuild) {
          autoQueuedCount++;
          autoQueuedFleet.StarShips[p.StarShipTypeLastBuilt].push(
          ...
          unableToAutoQueueFleet.StarShips[p.StarShipTypeLastBuilt].push(
            new Astriarch.Fleet.StarShip(
              p.StarShipTypeLastBuilt,
              p.StarShipCustomShipLastBuilt,
              data.advantageAgainst,
    ...
    if (autoQueuedCount || unableToAutoQueueCount) {
      ...
      if (unableToAutoQueueCount) {
        ...
        autoBuiltStarshipsMessage +=
          "Unable to auto-queue: " +
          unableToAutoQueueFleet.ToString() +
          ", need" +
          (goldShortage ? " " + goldShortage + " Gold" : "") +
          (oreShortage ? " " + oreShortage + " Ore" : "") +
          (iridiumShortage ? " " + iridiumShortage + " Iridium" : "");
      }
      var endOfTurnMessage = new Astriarch.SerializableTurnEventMessage(
        Astriarch.TurnEventMessage.TurnEventMessageType.ResourcesAutoSpent,
        focusPlanet,
        autoBuiltStarshipsMessage
      );
  ...
  buildPlayerPlanetImprovements: function(gameModel, /*Player*/ player) {
    //returns List<TurnEventMessage>
    ...
    for (var i in player.OwnedPlanets) {
      ...
      if (buildQueueEmptyObject["buildQueueEmpty"]) {
    ...
    if (planetNameBuildQueueEmptyList.length > 0) {
      endOfTurnMessages.push(
        new Astriarch.SerializableTurnEventMessage(
          Astriarch.TurnEventMessage.TurnEventMessageType.BuildQueueEmpty,
    ...
    for (var i in player.OwnedPlanets) {
      ...
      if (p.PlanetHappiness == Astriarch.Planet.PlanetHappinessType.Normal) {
        ...
        if (protestingCitizenCount >= p.Population.length / 2) {
          ...
          eotMessages.push(
            new Astriarch.SerializableTurnEventMessage(
              Astriarch.TurnEventMessage.TurnEventMessageType.CitizensProtesting,
              p,
  ...
  growPlayerPlanetPopulation: function(/*Player*/ player) {
    //returns List<SerializableTurnEventMessage>
    var endOfTurnMessages = []; // List<SerializableTurnEventMessage>
    ...
    for (var i in player.OwnedPlanets) {
      ...
      if (growthRate > 0) {
        ...
        if (lastCitizen.PopulationChange >= 1.0) {
          ...
          endOfTurnMessages.push(
            new Astriarch.SerializableTurnEventMessage(
              Astriarch.TurnEventMessage.TurnEventMessageType.PopulationGrowth,
              p,
  ...
  getAdditionalPlayerPoints: function(gameModel, player, opponentOptions, /*bool*/ playerWon) {
    ...
    var additionalPoints = Astriarch.ServerController.calculateAdditionalPoints(
      points,
      ownedPlanets,
      totalPopulation,
      maxPopulation,
    ...
    console.log(
      ...
      difficultyRating,
      "percentageOwned:",
      ownedPlanets / (totalSystems * planetsPerSystem),
      "ownedPlanets:",
      ownedPlanets,
      "totalPopulation:",
      totalPopulation,
  ...
  calculateAdditionalPoints: function(
    points,
    ownedPlanets,
    totalPopulation,
    maxPopulation,
