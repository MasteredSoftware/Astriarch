...
describe('#TradingCenter', function () {
	...
	describe('executeTrade()', function () {
		...
		it('should not execute a sell food trade for player1 given that player has insufficient resources', function (done) {
			var tradeAmount = 20;
			var trade = new Astriarch.TradingCenter.Trade(-1, player1.HomePlanetId, Astriarch.TradingCenter.TradeType.SELL, Astriarch.TradingCenter.ResourceType.FOOD, tradeAmount, Astriarch.TradingCenter.OrderType.MARKET, null);
		...
		it('should not execute a buy food trade for player1 given that player has insufficient gold', function (done) {
			var tradeAmount = 20;
			var trade = new Astriarch.TradingCenter.Trade(-1, player1.HomePlanetId, Astriarch.TradingCenter.TradeType.BUY, Astriarch.TradingCenter.ResourceType.FOOD, tradeAmount, Astriarch.TradingCenter.OrderType.MARKET, null);
			var homePlanet = gameModel.getPlanetById(player1.HomePlanetId);
			var executedStatus = gameModel.TradingCenter.executeTrade(gameModel, player1, homePlanet, trade);
			executedStatus.executed.should.equal(false);
	...
	describe('executeCurrentTrades()', function () {
		...
		it('should not execute all current trades when some trades are invalid', function (done) {
			var newTrades = [];
			newTrades.push(new Astriarch.TradingCenter.Trade(player1.Id, player1.HomePlanetId, Astriarch.TradingCenter.TradeType.BUY, Astriarch.TradingCenter.ResourceType.FOOD, 6, Astriarch.TradingCenter.OrderType.MARKET, null));
			newTrades.push(new Astriarch.TradingCenter.Trade(player1.Id, player1.HomePlanetId, Astriarch.TradingCenter.TradeType.BUY, Astriarch.TradingCenter.ResourceType.FOOD, 6, Astriarch.TradingCenter.OrderType.MARKET, null));
			gameModel.TradingCenter.currentTrades = gameModel.TradingCenter.currentTrades.concat(newTrades);
			var endOfTurnMessagesByPlayerId = {};
			var executedStatusListByPlayerId = Astriarch.ServerController.executeCurrentTrades(gameModel, endOfTurnMessagesByPlayerId);
