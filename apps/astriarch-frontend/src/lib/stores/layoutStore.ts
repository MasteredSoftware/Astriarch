...
export const layoutDimensions = derived(
	...
	([$layoutMode, $dimensions]) => {
		...
		const SIDEBAR_WIDTH = 600; // Fixed sidebar width in landscape mode
		const PANEL_MARGIN = 16; // Margin around panels (mx-4 = 16px)
		const BOTTOM_NAV_HEIGHT = 48; // Height of bottom navigation bar

		if ($layoutMode === 'landscape') {
			// Landscape mode: sidebar on right (600px), full height canvas on left (remaining space)
			return {
				canvasWidth: $dimensions.width - SIDEBAR_WIDTH - PANEL_MARGIN * 2, // Account for margins
