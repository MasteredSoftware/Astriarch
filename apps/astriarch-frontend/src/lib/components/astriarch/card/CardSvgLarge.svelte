<script lang="ts">
	...
	const disabledFilterId = `filter0_d_4_23115_${uid}`;
	const enabledFilterId = `filter0_d_122_20636_${uid}`;
</script>
...
{#if !enabled}
	<svg
		...
		fill="none"
	>
		<g filter="url(#{disabledFilterId})" transform="translate(-40,-20)">
			<path
...
{:else}
	<svg
		...
		fill="none"
	>
		<g filter="url(#{enabledFilterId})" transform="translate(-19,-20)">
			<path
		...
		<defs>
			...
			>
				...
				<feBlend
					mode="normal"
					in="SourceGraphic"
					in2="effect1_dropShadow_122_20636"
					result="shape"
				/>
			</filter>
		</defs>
