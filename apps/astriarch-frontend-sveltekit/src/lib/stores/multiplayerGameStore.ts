...
function createMultiplayerGameStore() {
  ...
  return {
    ...
    // Game state actions
    setGameModel: (gameModel: GameModelData) => update(store => ({
      ...store,
      gameModel
    })),

    setClientModel: (clientModel: ClientModelData) => update(store => ({
      ...store,
      clientModel
    })),
...
export const currentPlanet = derived(
  ...
  ($store) => {
    ...
    return $store.gameModel.modelData.planets.find(p => p.id.toString() === $store.selectedPlanet) || null;
  }
);
...
export const playerPlanets = derived(
  ...
  ($store) => {
    ...
    const currentPlayer = $store.gameModel.modelData.players.find(p => p.id === $store.currentPlayer);
    if (!currentPlayer) return [];
    
    // Filter planets by owned planet IDs
    return $store.gameModel.modelData.planets.filter(planet => 
...
export const multiplayerResourceData = derived(
  multiplayerGameStore,
  ($store) => {
...
export const multiplayerPopulation = derived(
  multiplayerGameStore,
  ($store) => {
