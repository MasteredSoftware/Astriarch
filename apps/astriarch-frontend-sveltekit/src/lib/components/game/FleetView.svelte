		...
		<div class="fleet-content">
				...
				<div class="info-grid">
					...
					{#if selectedFleet.destination}
						...
						<div class="info-item">
							...
							<span class="value"
								>{new Date(selectedFleet.destination.arrivalTime).toLocaleString()}</span
							>
						</div>
					{/if}
				</div>
			...
			<Card class="ship-composition">
				...
				<div class="fleet-total">
					<strong
						>Total Ships: {Object.values(selectedFleet.ships).reduce(
							(sum, count) => sum + count,
							0
						)}</strong
					>
			...
			<Card class="fleet-actions">
				...
				<div class="actions-grid">
					...
					<Button variant="secondary" disabled>🔄 Change Orders</Button>
					<Button variant="secondary" disabled>⏸️ Hold Position</Button>
					<Button variant="destructive" disabled>💥 Scuttle Fleet</Button>
				</div>
		...
		<div class="no-fleet">
			<Card>
				<div class="no-fleet-content">
					...
					<p>
						Select a fleet from the galaxy map or sidebar to view its details and issue commands.
					</p>
				</div>
			</Card>
		</div>
			...
			<div class="dialog-content">
					...
					<div class="orders-grid">
						{#each Object.entries(ORDER_TYPES) as [orderKey, orderInfo]}
							<label class="order-option">
								<input type="radio" bind:group={fleetOrders} value={orderKey} />
				...
				<div class="form-section">
					...
					<div class="selection-actions">
						<Button variant="secondary" on:click={setAllShips}>Select All Ships</Button>
						<Button
							variant="secondary"
							on:click={() =>
								(sendShips = { scouts: 0, destroyers: 0, cruisers: 0, battleships: 0 })}
						>
							Clear Selection
						</Button>
					</div>
			...
			<div class="dialog-actions">
				...
				<Button
					...
					disabled={!targetPlanetId ||
						Object.values(sendShips).reduce((sum, count) => sum + count, 0) === 0}
				>
					Send Fleet
				</Button>
			</div>
