<script lang="ts">
	...
	function getResearchProgress(researchType: string): number {
		...
		return Math.min(
			95,
			researchAllocations[researchType as keyof typeof researchAllocations] * 0.8
		);
	}
</script>
		...
		<div class="research-content">
			...
			<Card class="allocation-card">
				...
				<p class="allocation-description">
					Allocate your research resources to different fields. Higher allocation means faster
					progress.
				</p>
				...
				<!-- Total Allocation -->
				{#if true}
					{@const totalAllocation = Object.values(researchAllocations).reduce(
						(sum, val) => sum + val,
						0
					)}
