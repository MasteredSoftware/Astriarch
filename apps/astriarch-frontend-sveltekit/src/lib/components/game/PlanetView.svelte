<script lang="ts">
	...
	const BUILDINGS = {
		farm: {
			name: 'Farm',
			cost: { ore: 10, iridium: 0 },
			description: 'Produces food for population growth'
		},
		mine: {
			name: 'Mine',
			cost: { ore: 15, iridium: 5 },
			description: 'Extracts ore and iridium from planet'
		},
		factory: {
			name: 'Factory',
			cost: { ore: 20, iridium: 10 },
			description: 'Builds ships and advanced structures'
		},
		spacePlatform: {
			name: 'Space Platform',
			cost: { ore: 50, iridium: 25 },
			description: 'Defensive platform and fleet base'
		}
	};
	...
	const SHIPS = {
		scout: {
			name: 'Scout',
			cost: { ore: 5, iridium: 2 },
			description: 'Fast exploration and reconnaissance ship'
		},
		destroyer: {
			name: 'Destroyer',
			cost: { ore: 15, iridium: 8 },
			description: 'Basic combat vessel'
		},
		cruiser: { name: 'Cruiser', cost: { ore: 25, iridium: 15 }, description: 'Heavy combat ship' },
		battleship: {
			name: 'Battleship',
			cost: { ore: 40, iridium: 25 },
			description: 'Powerful capital ship'
		}
	};
...
{#if planet}
	<div class="planet-view-overlay">
		<Card
			style="width: 90%; max-width: 800px; max-height: 90%; overflow-y: auto; background: var(--background); border: 2px solid var(--border);"
		>
					...
					<div class="info-section">
						...
						<div class="status-grid">
							<div class="status-item">
								...
								<span class="value"
									>{planet.playerId ? planet.playerName || planet.playerId : 'Neutral'}</span
								>
							</div>
							<div class="status-item">
							...
							<div class="building-item">
								<div class="building-header">
									...
									<span class="building-count"
										>{planet.buildings[buildingType as keyof typeof planet.buildings]}</span
									>
								</div>
								...
								{#if canBuild}
									<div class="building-actions">
										<Button
											...
											disabled={planet.resources.ore < buildingInfo.cost.ore ||
												planet.resources.iridium < buildingInfo.cost.iridium}
										>
											Build
										</Button>
