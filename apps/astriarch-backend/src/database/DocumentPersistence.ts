...
export class DocumentPersistence {
   ...
   */
  static async saveDocuments<T extends Document>(
    documents: { doc: T; mixedFields?: string[] }[]
  ): Promise<T[]> {
    const savePromises = documents.map(({ doc, mixedFields }) => 
      DocumentPersistence.saveDocument(doc, mixedFields)
    );
