...
var wsSend = function(ws, message, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        console.error("Problem in wsSend:", err);
      }
    };
...
var sendChatRoomSessionListUpdates = function(ws, sessionId, chatRoomWithSessions) {
  ...
  var cleanSessions = [],
    session = null; //This doesn't include other player session ids for security
  for (var s = 0; s < chatRoomWithSessions.sessions.length; s++) {
    session = chatRoomWithSessions.sessions[s];
    cleanSessions.push({
      playerName: session.playerName,
      playerNumber: session.playerNumber,
      dateJoined: session.dateJoined
    });
  }
  var messageForPlayers = new Astriarch.Shared.Message(Astriarch.Shared.MESSAGE_TYPE.CHAT_ROOM_SESSIONS_UPDATED, {
...
var addMessageToChatRoomAndBroadcastToOtherMembers = function(gameId, chatLogMessage, sessionId) {
  gameController.AddMessageToChatRoom(gameId, chatLogMessage, function(err, doc) {
    ...
    var messageForPlayers = new Astriarch.Shared.Message(Astriarch.Shared.MESSAGE_TYPE.CHAT_MESSAGE, {
      messageType: chatLogMessage.messageType,
      text: chatLogMessage.text,
      sentByPlayerName: chatLogMessage.sentByPlayerName,
      sentByPlayerNumber: chatLogMessage.sentByPlayerNumber,
