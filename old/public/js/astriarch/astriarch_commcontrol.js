Astriarch.CommControl = {
  ...
  setPlayerNameDivAndLocalStorageToNewName: function(newName) {
    Astriarch.CommControl.playerName = newName;
  ...
  sendTextMessage: function() {
    ...
    if (text && text.trim()) {
      ...
      var message = {
        ...
        text: text,
        sentByPlayerName: this.playerName,
        sentByPlayerNumber: this.playerNumber,
  ...
  joinChatRoom: function(playerName, playerNumber) {
    ...
    Astriarch.server_comm.sendMessage({
      type: Astriarch.Shared.MESSAGE_TYPE.CHAT_MESSAGE,
      payload: {
        messageType: Astriarch.Shared.CHAT_MESSAGE_TYPE.PLAYER_ENTER,
        sentByPlayerName: playerName,
        sentByPlayerNumber: playerNumber
      }
    });
    this.chatLog.html("");
  },
  ...
  appendMessages: function(messages) {
    ...
    for (var i = 0; i < messages.length; i++) {
      ...
      var messageFromClass = message.sentByPlayerNumber
        ? "messagePlayer" + message.sentByPlayerNumber
        : "messagePlayerLobby";
      if (message.messageType == Astriarch.Shared.CHAT_MESSAGE_TYPE.TEXT_MESSAGE) {
        html +=
          '<div class="chatLogMessage"><span class="messageTime">' +
          messageDateString +
          '</span><span class="' +
          messageFromClass +
          '">' +
          message.sentByPlayerName +
          ": </span>" +
          message.text +
          "</div>";
      } else {
        var text =
          message.messageType == Astriarch.Shared.CHAT_MESSAGE_TYPE.PLAYER_ENTER
            ? " Arrived"
            : message.messageType == Astriarch.Shared.CHAT_MESSAGE_TYPE.PLAYER_DISCONNECT
              ? " Disconnected"
              : " Left";
        html +=
          '<div class="chatLogMessage"><span class="messageTime">' +
          messageDateString +
          '</span><span class="messageSystem">' +
          message.sentByPlayerName +
          text +
          "</span></div>";
      }
