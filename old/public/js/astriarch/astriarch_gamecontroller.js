...
Astriarch.GameController.ResetView = function(serializableClientModel) {
  ...
  Astriarch.ClientGameModel = Astriarch.ClientModelInterface.GetClientModelFromSerializableClientModel(
    serializableClientModel,
    gameGrid
  );
...
Astriarch.GameController.SetupViewFromGameModel = function() {
  ...
  for (var q in Astriarch.ClientGameModel.GameGrid.Quadrants) {
    //add to canvas layer
    Astriarch.View.CanvasPlayfieldLayer.addChild(
      new Astriarch.Grid.DrawnRect(Astriarch.ClientGameModel.GameGrid.Quadrants[q])
    );
  }
...
Astriarch.GameController.OnPlayerDestroyed = function(/*ClientPlayer)*/ clientPlayer) {
  var a = new Astriarch.Alert(
    clientPlayer.Name + " Destroyed",
    'Player: <span style="color:' + clientPlayer.Color.toString() + '">' + clientPlayer.Name + "</span> destroyed."
  );
};
...
Astriarch.GameController.OnGameOverMessageResponse = function(message) {
  ...
  Astriarch.GameOverControl.show(
    message.payload.winningSerializablePlayer,
    message.payload.playerWon,
...
Astriarch.GameController.RefreshTurnDisplay = function() {
  ...
  if (Astriarch.ClientGameModel.GameOptions.TurnTimeLimitSeconds && !Astriarch.GameController.gameOver) {
    ...
    turnTimer.stop(true).animate({ width: "100%" }, 500, "linear", function() {
      ...
      Astriarch.GameController.turnTimerTimeoutId = setTimeout(function() {
        Astriarch.View.NextTurn();
      }, Astriarch.ClientGameModel.GameOptions.TurnTimeLimitSeconds * 1000);
    });
  } else {
    turnTimer.hide();
  }
...
Astriarch.GameController.UpdateUIForEndTurnMessage = function(message) {
  Astriarch.ClientGameModel = Astriarch.ClientModelInterface.GetClientModelFromSerializableClientModel(
    ...
    Astriarch.ClientGameModel.GameGrid
  );
  ...
  for (var s in serializableEndOfTurnMessages) {
    endOfTurnMessages.push(
      Astriarch.ClientModelInterface.GetTurnEventMessageFromSerializableTurnEventMessage(
        serializableEndOfTurnMessages[s],
        Astriarch.ClientGameModel.GameGrid
      )
    );
  }
  ...
  if (Astriarch.PlayerGameOptions.ShowPlanetaryConflictPopups) {
    for (var i in endOfTurnMessages) {
      ...
      if (
        tem.Type == Astriarch.TurnEventMessage.TurnEventMessageType.AttackingFleetLost ||
  ...
  var homePlanet = Astriarch.ClientGameModel.MainPlayer.GetPlanetIfOwnedByPlayer(
    Astriarch.ClientGameModel.MainPlayer.HomePlanetId
  );
  if (homePlanet && window.tour.enabled) {
    ...
    if (
      (factoryCount == 1 && window.tour.step == 48) ||
    ...
    if (
      Astriarch.CountObjectKeys(Astriarch.ClientGameModel.MainPlayer.KnownClientPlanets) >= 4 &&
      window.tour.step == 63
    ) {
      window.tour.jqElm.joyride("resume");
    }
...
Astriarch.GameController.TurnSummaryMessageListBoxItem = JSListBox.Item.extend({
  ...
  onDblClick: function() {
    ...
    if (
      this.EventMessage.Type == Astriarch.TurnEventMessage.TurnEventMessageType.AttackingFleetLost ||
