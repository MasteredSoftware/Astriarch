...
Astriarch.Research = function() {
  ...
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.NEW_SHIP_TYPE_DEFENDER, 0);
  this.researchProgressByType[
    Astriarch.Research.ResearchType.NEW_SHIP_TYPE_SCOUT
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.NEW_SHIP_TYPE_SCOUT, 0);
  this.researchProgressByType[
    Astriarch.Research.ResearchType.NEW_SHIP_TYPE_DESTROYER
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.NEW_SHIP_TYPE_DESTROYER, 0);
  this.researchProgressByType[
    Astriarch.Research.ResearchType.NEW_SHIP_TYPE_CRUISER
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.NEW_SHIP_TYPE_CRUISER, 0);
  this.researchProgressByType[
    Astriarch.Research.ResearchType.NEW_SHIP_TYPE_BATTLESHIP
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.NEW_SHIP_TYPE_BATTLESHIP, 0);
  this.researchProgressByType[
    Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_ATTACK
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_ATTACK, 0, {
    chance: 0
  });
  this.researchProgressByType[
    Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_DEFENSE
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_DEFENSE, 0, {
    chance: 0
  });
  this.researchProgressByType[
    Astriarch.Research.ResearchType.PROPULSION_IMPROVEMENT
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.PROPULSION_IMPROVEMENT, 0, {
    percent: 1.0
  });
  this.researchProgressByType[
    Astriarch.Research.ResearchType.BUILDING_EFFICIENCY_IMPROVEMENT_FARMS
  ] = new Astriarch.Research.ResearchTypeProgress(
    Astriarch.Research.ResearchType.BUILDING_EFFICIENCY_IMPROVEMENT_FARMS,
    0,
    { percent: 1.0 }
  );
  ...
  this.researchProgressByType[
    Astriarch.Research.ResearchType.BUILDING_EFFICIENCY_IMPROVEMENT_COLONIES
  ] = new Astriarch.Research.ResearchTypeProgress(
    Astriarch.Research.ResearchType.BUILDING_EFFICIENCY_IMPROVEMENT_COLONIES,
    0,
    { percent: 1.0 }
  ...
  this.researchProgressByType[
    Astriarch.Research.ResearchType.SPACE_PLATFORM_IMPROVEMENT
  ] = new Astriarch.Research.ResearchTypeProgress(Astriarch.Research.ResearchType.SPACE_PLATFORM_IMPROVEMENT, 0, {
    max: 1
  });
...
Astriarch.Research.prototype.estimateTurnsRemainingInQueue = function(goldAmountAtMaxPercent) {
  ...
  return this.researchTypeInQueue
    ? Math.ceil(
        this.researchProgressByType[this.researchTypeInQueue].GetResearchLevelData().researchCostToNextLevel /
          researchAmountEarned
      )
    : 999;
};
...
Astriarch.Research.prototype.nextTurn = function(goldAmountAtMaxPercent) {
  ...
  if (this.researchTypeInQueue) {
    ...
    returnObj.researchAmountEarned += Astriarch.NextRandom(0, returnObj.researchAmountEarned * 0.1);

    var rtpInQueue = this.researchProgressByType[this.researchTypeInQueue];
    var levelIncrease = rtpInQueue.setResearchPointsCompleted(
      rtpInQueue.researchPointsCompleted + returnObj.researchAmountEarned
    );
    if (levelIncrease) {
      //we've gained a level

      returnObj.endOfTurnMessages.push(
        new Astriarch.TurnEventMessage(
          Astriarch.TurnEventMessage.TurnEventMessageType.ResearchComplete,
          null,
          "Our Scientists and Engineers have finished researching and developing: " + rtpInQueue.ToString()
        )
      );
    ...
    if (this.researchPercent) {
      returnObj.endOfTurnMessages.push(
        new Astriarch.TurnEventMessage(
          Astriarch.TurnEventMessage.TurnEventMessageType.ResearchQueueEmpty,
          null,
          "Our Scientists and Engineers are idle, R&D points were used for tax collection instead."
        )
      );
    }
  }
...
Astriarch.Research.ResearchTypeProgress.prototype.GetResearchLevelData = function() {
  var researchCostData = {
    currentResearchLevel: this.currentResearchLevel + 1,
    researchCostToNextLevel: 0,
    percentComplete: 0
  };
  var totalCostAtCurrentLevel =
    researchCostData.currentResearchLevel === 0
      ? 0
      : this.researchLevelCosts[researchCostData.currentResearchLevel - 1];
  ...
  researchCostData.percentComplete =
    (this.researchPointsCompleted - totalCostAtCurrentLevel) / (totalCostToNextLevel - totalCostAtCurrentLevel);
...
Astriarch.Research.ResearchTypeProgress.prototype.GetCurrentLevelDataString = function() {
  ...
  switch (this.type) {
    ...
    case Astriarch.Research.ResearchType.NEW_SHIP_TYPE_BATTLESHIP:
      returnString +=
        "Advantage over: " +
        Astriarch.GameTools.StarShipTypeToFriendlyName(this.data.advantageAgainst) +
        ", Disadvantage over: " +
        Astriarch.GameTools.StarShipTypeToFriendlyName(this.data.disadvantageAgainst);
      break;
    case Astriarch.Research.ResearchType.COMBAT_IMPROVEMENT_ATTACK:
...
Astriarch.Research.ResearchTypeProgress.prototype.GetFriendlyName = function() {
  ...
  switch (this.type) {
    case Astriarch.Research.ResearchType.NEW_SHIP_TYPE_DEFENDER:
      defaultName =
        "Custom " + Astriarch.GameTools.StarShipTypeToFriendlyName(Astriarch.Fleet.StarShipType.SystemDefense);
      break;
    case Astriarch.Research.ResearchType.NEW_SHIP_TYPE_SCOUT:
