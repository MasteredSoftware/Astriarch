Astriarch.ResearchControl = {
  ...
  init: function() {
    ...
    Astriarch.ResearchControl.jqElm.sliderResearchPercent.slider({
      value: 0,
      step: 10,
      min: 0,
      max: 100,
      slide: function(event, ui) {
        Astriarch.ResearchControl.SliderResearchPercentValueChanged(ui.value);
      }
    });

    Astriarch.ResearchControl.jqElm.buttonSubmitCustomShip.button();
    Astriarch.ResearchControl.jqElm.buttonSubmitCustomShip.click(function() {
      Astriarch.ResearchControl.ButtonSubmitCustomShipClick();
    });

    Astriarch.ResearchControl.jqElm.buttonIncreaseResearch.button({
      icons: { primary: "icon-16x16-research" },
      text: false
    });
    Astriarch.ResearchControl.jqElm.buttonIncreaseResearch.click(function() {
    ...
    Astriarch.ResearchControl.jqElm.buttonIncreaseTaxes.button({
      icons: { primary: "icon-16x16-credits" },
      text: false
    });
    Astriarch.ResearchControl.jqElm.buttonIncreaseTaxes.click(function() {
      var newSliderValue = Astriarch.ResearchControl.jqElm.sliderResearchPercent.slider("value") - 10;
      Astriarch.ResearchControl.SliderResearchPercentValueChanged(newSliderValue);
    });

    Astriarch.ResearchControl.researchTypeProgressCardList = new JSCardList({
      containerSelector: "ResearchTypeProgressCardList",
      multiselect: false
    });

    Astriarch.ResearchControl.dialog = new Astriarch.Dialog(
      "#researchControlDialog",
      "Research and Development",
      470,
      425,
      Astriarch.ResearchControl.OKClose
    );
  },
    ...
    if (Astriarch.ClientGameModel.MainPlayer.Research.researchTypeInQueue) {
      var rtpInQueue =
        Astriarch.ClientGameModel.MainPlayer.Research.researchProgressByType[
          Astriarch.ClientGameModel.MainPlayer.Research.researchTypeInQueue
        ];
      projectStatusHtml =
        "Currently Researching: " +
        rtpInQueue.ToString(true) +
        ", Estimated Turns Remaining: " +
        Astriarch.ClientGameModel.MainPlayer.Research.estimateTurnsRemainingInQueue(
          Astriarch.ClientGameModel.MainPlayer.GetTaxRevenueAtMaxPercent()
        );
    }
  ...
  SliderResearchPercentValueChanged: function(value) {
    ...
    Astriarch.server_comm.sendMessage({
      type: Astriarch.Shared.MESSAGE_TYPE.ADJUST_RESEARCH_PERCENT,
      payload: { researchPercent: percent }
    });
  ...
  buildCustomShipComboBoxes: function(selectedItem) {
    var defenderOption =
      selectedItem.ResearchTypeProgress.type == Astriarch.Research.ResearchType.NEW_SHIP_TYPE_DEFENDER
        ? ""
        : '<option value="1">Defenders</option>';
    var mobileShipOptions =
      '<option value="2" selected="">Scouts</option><option value="3">Destroyers</option><option value="4">Cruisers</option><option value="5"">Battleships</option>';
    Astriarch.ResearchControl.jqElm.researchNewShipTypeAdvantageBox.html(
      '<select id="ResearchNewShipTypeAdvantageComboBox">' + defenderOption + mobileShipOptions + "</select>"
    );
    Astriarch.ResearchControl.jqElm.researchNewShipTypeDisadvantageBox.html(
      '<select id="ResearchNewShipTypeDisadvantageComboBox">' + defenderOption + mobileShipOptions + "</select>"
    );
  ...
  SubmitResearchTypeProgress: function(type, data) {
    ...
    Astriarch.server_comm.sendMessage({
      type: Astriarch.Shared.MESSAGE_TYPE.SUBMIT_RESEARCH_ITEM,
      payload: { researchItem: ri }
    });
...
Astriarch.ResearchControl.ResearchTypeProgressCardListItem = JSCardList.Item.extend({
  ...
  render: function() {
    this.enabled = false;
    if (!this.CanResearch) this.Foreground = "darkgray";
    else {
    ...
    var element =
      '<span class="rtpItem">' +
      '<span class="rtpItemName">' +
      text +
      "</span>" +
      '<div class="rtpItemImg ' +
      imageClassName +
      '" />' +
      '<div class="rtpPercentCompleteContainer"><div class="rtpHealthBar" style="background:#369;height:' +
      (this.CanResearch ? researchLevelData.percentComplete * 100 : 0) +
      '%" /></div>' +
      '<span class="rtpItemLevel">' +
      (researchLevelData.currentResearchLevel || "") +
      "</span>" +
      "</span>";

    return (
      '<a class="rtpItemAnchor" ' +
      (this.enabled ? 'href="#" ' : "") +
      'title="' +
      this.Tooltip +
      '" style="color:' +
