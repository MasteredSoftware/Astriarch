Astriarch.TradingControl = {
  ...
  init: function() {
    ...
    $("#SliderTradeResourceAmount").slider({
      value: 0,
      step: 1,
      min: 0,
      max: 10,
      slide: Astriarch.TradingControl.SliderTradeResourceAmountValueChanged
    });

    $("#ButtonTradesSubmittedRetractSelectedTrade").button({
      icons: { primary: "icon-16x16-build-queue-remove" },
      text: false
    });
    $("#ButtonTradesSubmittedRetractSelectedTrade").button("disable");
    $("#ButtonTradesSubmittedRetractSelectedTrade").click(function() {
    ...
    Astriarch.TradingControl.jqElm.buttonSubmitTrade.click(function() {
    ...
    $("input[name=TradeTypeRadioGroup]:radio").change(function() {
      Astriarch.TradingControl.setSliderPropertiesBasedOnNewTradeOptions();
    });
    $("input[name=TradeResourceRadioGroup]:radio").change(function() {
      Astriarch.TradingControl.setSliderPropertiesBasedOnNewTradeOptions();
    });

    Astriarch.TradingControl.tradesSubmittedListBox = new JSListBox({ containerSelector: "TradesSubmittedListBox" });

    Astriarch.TradingControl.dialog = new Astriarch.Dialog(
      "#tradingControlDialog",
      "Galactic Trading Center",
      470,
      425,
      Astriarch.TradingControl.OKClose
    );
  },
  ...
  setPlayerResourceAmountAfterCurrentTrades: function() {
    ...
    this.playerResourceAmountsAfterTrades = {
      food: player.TotalFoodAmount(),
      ore: player.TotalOreAmount(),
      iridium: player.TotalIridiumAmount()
    };
  ...
  ButtonSubmitTradeClick: function() {
    ...
    var clientTrade = new Astriarch.TradingCenter.Trade(
      -1,
      this.planet.Id,
      radioOptions.tradeType,
      radioOptions.resourceType,
      amount,
      orderType,
      limitPrice
    );
    ...
    Astriarch.server_comm.sendMessage({
      type: Astriarch.Shared.MESSAGE_TYPE.SUBMIT_TRADE,
      payload: { trade: clientTrade }
    });
  ...
  ButtonRetractSelectedTradeClick: function() {
    ...
    if (o != null && index != null) {
      Astriarch.server_comm.sendMessage({
        type: Astriarch.Shared.MESSAGE_TYPE.CANCEL_TRADE,
        payload: { tradeIndex: index, planetId: this.planet.Id }
      });
  ...
  getSubmitTradeRadioButtonOptions: function() {
    ...
    radioOptions.tradeType =
      radioOptions.tradeType == "buy" ? Astriarch.TradingCenter.TradeType.BUY : Astriarch.TradingCenter.TradeType.SELL;
    radioOptions.resourceType = $("input:radio[name=TradeResourceRadioGroup]:checked").val();
    radioOptions.resourceType =
      radioOptions.resourceType == "food"
        ? Astriarch.TradingCenter.ResourceType.FOOD
        : radioOptions.resourceType == "ore"
