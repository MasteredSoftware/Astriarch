...
Astriarch.SavedGameInterface.findCircularReferences = function(obj, keyName, parentKeyName, hash) {
  ...
  if (hash.indexOf(obj) == -1) {
    hash.push(obj);
  } else alert("found circular reference: " + parentKeyName + "." + keyName);

  for (var key in obj) Astriarch.SavedGameInterface.findCircularReferences(obj[key], key, keyName, hash);
  return;
};
...
Astriarch.SavedGameInterface.getModelFromSerializableModel = function(/*Astriarch.SerializableModel*/ serializedModel) {
  ...
  newModel.TradingCenter.foodResource = Astriarch.SavedGameInterface.getTradingCenterResourceFromSerializableTradingCenterResource(
    serializedModel.TradingCenter.foodResource
  );
  newModel.TradingCenter.oreResource = Astriarch.SavedGameInterface.getTradingCenterResourceFromSerializableTradingCenterResource(
    serializedModel.TradingCenter.oreResource
  );
  newModel.TradingCenter.iridiumResource = Astriarch.SavedGameInterface.getTradingCenterResourceFromSerializableTradingCenterResource(
    serializedModel.TradingCenter.iridiumResource
  );
  ...
  for (var i in serializedModel.SerializablePlayers) {
    players.push(
      Astriarch.SavedGameInterface.getPlayerFromSerializedPlayer(
        newModel.GameGrid,
        serializedModel.SerializablePlayers[i],
        planetsById
      )
    );
  }
  ...
  for (var i in players) {
    ...
    clientPlayersById[player.Id] = new Astriarch.ClientPlayer(
      player.Id,
      player.Type,
      player.Name,
      player.Color,
...
Astriarch.SavedGameInterface.getTradingCenterResourceFromSerializableTradingCenterResource = function(stcr) {
  return new Astriarch.TradingCenter.Resource(
    stcr.type,
    stcr.amount,
    stcr.desiredAmount,
    stcr.priceMin,
...
Astriarch.SavedGameInterface.setPlayerLastKnownPlanetFleetStrength = function(
  /*Player*/ p,
  /*SerializablePlayer*/ sp,
  gameGrid,
  clientPlayersById
) {
  ...
  for (var j in sp.LastKnownPlanetSerializableFleetStrength) {
    ...
    var fleet = Astriarch.SavedGameInterface.getFleetFromSerializableFleet(
      lastKnownOwner,
      sLKF.SerializableFleet,
      gameGrid
    );
...
Astriarch.SavedGameInterface.getPlanetFromSerializedPlanet = function(sp, gameGrid) {
  ...
  p.PlanetaryFleet = Astriarch.SavedGameInterface.getFleetFromSerializableFleet(
    null,
    sp.PlanetarySerializableFleet,
    gameGrid
  );

  //populate outgoingFleets
  for (var j in sp.OutgoingSerializableFleets) {
    p.OutgoingFleets.push(
      Astriarch.SavedGameInterface.getFleetFromSerializableFleet(null, sp.OutgoingSerializableFleets[j], gameGrid)
    );
  }
...
Astriarch.SavedGameInterface.getPlayerFromSerializedPlayer = function(
  gameGrid,
  /*SerializablePlayer*/ sp,
  planetsById,
  clientPlanetsById
  ...
  for (var t in p.Research.researchProgressByType) {
    ...
    p.Research.researchProgressByType[t] = new Astriarch.Research.ResearchTypeProgress(
      rpt.type,
      rpt.researchPointsCompleted,
      rpt.data
    );
  }
  ...
  for (var j in sp.SerializableFleetsInTransit) {
    p.FleetsInTransit.push(
      Astriarch.SavedGameInterface.getFleetFromSerializableFleet(p, sp.SerializableFleetsInTransit[j], gameGrid)
    );
  }
...
Astriarch.SavedGameInterface.getPlanetProductionItemFromSerializedPlanetProductionItem = function(
  /*PlanetProductionItem*/ sppi
) {
  var ppi = null;
  ...
  } else if (sppi.PlanetProductionItemType == Astriarch.Planet.PlanetProductionItemType.StarShipInProduction) {
    //it's a ship
    ppi = new Astriarch.Planet.StarShipInProduction(
      sppi.Type,
      sppi.CustomShip,
      sppi.AdvantageAgainstType,
      sppi.DisadvantageAgainstType
    );
  } else if (sppi.PlanetProductionItemType == Astriarch.Planet.PlanetProductionItemType.PlanetImprovementToDestroy) {
...
Astriarch.SavedGameInterface.getFleetFromSerializableFleet = function(
  /*ClientPlayer*/ cp,
  /*Astriarch.SerializableFleet*/ serializedFleet,
  gameGrid
) {
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.SystemDefense]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.SystemDefense][i]
    );
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Scout]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Scout][i]
    );
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Destroyer]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Destroyer][i]
    );
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Cruiser]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Cruiser][i]
    );
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Battleship]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.Battleship][i]
    );
  ...
  for (var i in serializedFleet.StarShips[Astriarch.Fleet.StarShipType.SpacePlatform]) {
    var ship = extend(
      ...
      serializedFleet.StarShips[Astriarch.Fleet.StarShipType.SpacePlatform][i]
    );
  ...
  if (serializedFleet.travelingFromHexMidPoint) {
    ...
    f.SetDestination(
      ...
      travelingFromHex,
      destinationHex,
      serializedFleet.ParsecsToDestination,
      totalTravelDistance
    );
  }
  return f;
...
Astriarch.SerializablePlayer = function(/*Astriarch.Player*/ player) {
  ...
  for (var i in player.LastKnownPlanetFleetStrength) {
    this.LastKnownPlanetSerializableFleetStrength[i] = new Astriarch.Fleet.SerializableLastKnownFleet(
      player.LastKnownPlanetFleetStrength[i]
    );
  }
  ...
  for (var i in player.FleetsInTransit) {
    this.SerializableFleetsInTransit.push(
      new Astriarch.SerializableFleet(player.FleetsInTransit[i], this.Type == Astriarch.Player.PlayerType.Human)
    );
  }
...
Astriarch.SerializablePlanet = function(/*Astriarch.Planet*/ planet) {
  ...
  for (var i in planet.OutgoingFleets) {
    if (planet.Owner) {
      this.OutgoingSerializableFleets.push(
        new Astriarch.SerializableFleet(
          planet.OutgoingFleets[i],
          planet.Owner.Type == Astriarch.Player.PlayerType.Human
        )
      );
    }
  }
...
Astriarch.SavedGameInterface.makePlanetProductionItemSerializable = function(/*PlanetProductionItem*/ sppi) {
  ...
  } else if (sppi instanceof Astriarch.Planet.StarShipInProduction) {
    //it's a ship
    sppi.PlanetProductionItemType = Astriarch.Planet.PlanetProductionItemType.StarShipInProduction;
  } else if (sppi instanceof Astriarch.Planet.PlanetImprovementToDestroy) {
    //it is a destroy improvement request
